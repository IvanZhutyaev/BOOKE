...
document.getElementById('inventory-close').addEventListener('click', () => {
    ...
    setActiveNavButton(0); // сбрасываем активное состояние - возвращаемся в главное меню
});
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Обработчик кнопки закрытия инвентаря
    const inventoryCloseBtn = document.getElementById('inventory-close');
    if (inventoryCloseBtn) {
        inventoryCloseBtn.addEventListener('click', () => {
            console.log('Кнопка закрытия инвентаря нажата!');
            document.getElementById('inventory-panel').style.display = 'none';
            setActiveNavButton(0); // сбрасываем активное состояние - возвращаемся в главное меню
        });
    } else {
        console.error('Кнопка inventory-close не найдена!');
    }
});
